            ...
            if (keyPressed) {
                for (yRow = 0; yRow < resizeWndWidth; yRow++) {
                    for (xColumn = 0; xColumn < resizeWndHeight; xColumn++)
                    {
                        ...
                        unsigned int finalDsColor = dsG + dsB + dsR;
                        //set final ds color back to the same pixel
                        COLORREF dsPixelColor = RGB(finalDsColor, finalDsColor, finalDsColor);
                        SetPixel(hdc, xColumn, yRow, dsPixelColor);
                    }
                }
            }
